'''
Upper level interface for Espec Corp. GL Controller (original command set)

:copyright: (C) Espec North America, INC.
:license: MIT, see LICENSE for more details.
:authors: Paul Nong-Laolam
:implemented for GL controller 
'''
...
class EspecGLC(ControllerInterface):
    '''
    A class for interfacing with Espec controllers (GL)
    ...
    def __init__(self, **kwargs):
        ...
        self.lp_exmsg = 'The GL controller only supports 2 loops (%d:temperature,%d:humidity)'%ttp
        self.cs_exmsg = 'The GL controller can only have loop %d as cascade' % self.temp
        self.alarms = 27
        self.profiles = True
        self.events = 12
    ...
    def connect(self):
        ...
        '''
        self.client = GLC(self.interface, **self.connect_args)
    ...
    def cached(self, func, *args, **kwargs):
        '''
        The GLC returns multiple parameters with each command. The commands responses will be
    ...
    def get_prgm_counter(self):
        ...
        try: 
            # P300/GLC both have two counters that can operate
            ...
            # ES102, unlike P300/GLC, has only one counter with a single loop.
            # Loop name: counter_a; assigned to be consistent with that used for GLC.  
            ret = [
                {
                    'name':'A',
                    'remaining':prgm_mon['counter_a'],
    ...
    def get_prgm_time(self, pgm=None):
        ...
        try: # set for P300 and GLC which have two counter loops
            ...
            # correct for GLC not resetting the nested counter until it hits counter.start
            ...
            textmsg = 'Testing ES102 on especGLC.py...print out values of loops:'
            use_a = pgm['counter_a']['cycles'] != 0
            tta, tminutes = -1, 0
            count_a = pgms['counter_a']
            if use_a:
    ...
    def process_controller(self, update=True):
        if update:
            self.loops = 0
            self.cascades = 0
        #msg = 'GLC ' if self.client.read_rom().startswith('P3') else 'SCP-220 '
        msg = 'GLC ' if self.client.read_rom().startswith('P3') else 'SCP-220 ' if self.client.read_rom().startswith('JPC') else 'ES102 '
        try:
            if update:
