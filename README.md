...
* ```glc_runTCP.py```: Sample program to test, control and operate GL chamber. Default communication protocol is TCP/IP. Programmer may modify the protocol between TCP/IP, RS232 and RS485. However, serial communication protocol is option-dependent; it must also be enabled on the GLC chamber system.    
